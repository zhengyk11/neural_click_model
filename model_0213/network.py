...
class Network(nn.Module):
    def __init__(self, args, query_size, doc_size, vtype_size):
        ...
        self.doc_gru = nn.GRU(self.embed_size, self.embed_size, bidirectional=True,
                          batch_first=True, dropout=self.dropout_rate, num_layers=self.encode_gru_num_layer)
        self.gru = nn.GRU(self.embed_size*4, self.hidden_size,
                          batch_first=True, dropout=self.dropout_rate, num_layers=self.encode_gru_num_layer)
    ...
    def forward(self, query, doc, vtype, action):
        ...
        query_embed_state = torch.transpose(query_embed, 0, 1)[0:2].contiguous()
        new_doc_embed, _ = self.doc_gru(doc_embed, query_embed_state)
        # print new_doc_embed.size()
        gru_input = torch.cat((query_embed, new_doc_embed, vtype_embed, action_embed), dim=2)
